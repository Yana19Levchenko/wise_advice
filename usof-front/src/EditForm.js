...
function EditForm() {
  ...
  const editPost = async (e, post) => {
    ...
    try {
      const response = await axiosInstance.patch(`posts/${postId}`, addedData);
  ...
  const getData = async () => {
    ...
    try {
      ...
      const resp = await axiosInstance.get(`posts/${postId}/categories`);
      const tags = resp.data;
      const titles = resp.data.map(tag => tag.title);
      setSelectedCategories(titles);
  ...
  return (
	<form onSubmit={editPost} class='form edit-form' >
           <h1 id='form-header'>Edit Post</h1>
           ...
           <br/><br/><button id='login-btn' type="submit">Edit Post</button><br/>
