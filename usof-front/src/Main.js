...
function Main() {
  ...
  useEffect(() => {
    ...
    fetchCategories();
  }, [currentPage, sortCondition, filterStatus, selectedCategories, filterDate, navigate]);
    ...
    <header class="header">
       ...
       {isOpen && (
         <div class='menu menu-option'>
            {!isAuth ? (
                  <a class='top-option' href='/'>Home</a>
            ) : (
               <>
       ...
       <p id='user-desc'>
         {isAuth && decodedToken ? userData.login : '???'}<br />
         {isAuth && decodedToken ? userData.role : 'guest'}
       </p>
       {isAuth ? (
  ...
  <div>
    <img src={'/main_girl.png'} id='main-girl' alt=''/>
  </div>
